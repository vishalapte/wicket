...
provides three verbs over one year-sharded manifest: `catalog` (observe the
mailbox into the manifest) and `fetch` (download `.eml`), both read-only on the
mailbox over IMAP, and `report` (manifest summaries, no IMAP). It was lifted out of `factotum`; its consumers (factotum, meridian)
...
- **Layering is the contract.** `[tool.importlinter]` in `pyproject.toml` is the
  source of truth: one `layers` contract, per-verb `__main__` (face) → per-verb
  `api` → per-verb worker → `reconcile` → shared
  (`auth`/`domains`/`message`/`manifest`) → `config`, downward only (acyclic).
  The three verbs are independent siblings (no cross-imports). The face → api →
  lib routing (a face reaches the engine through its api, not the worker/shared
  directly) is the recommended **convention**, reviewed not walled: the contract gates
  defects (cycles, direction); it does not forbid
