...
Shared across `wicket.attachments` (attachment downloader) and
`wicket.fetch` (.eml archiver). Both authenticate to Gmail over IMAP
using an app password, not OAuth. The credential file is JSON of the form
...
def resolve_state_dir(override: str | os.PathLike[str] | None = None) -> Path:
    """Return the state directory, creating it owner-only (0700) if missing."""
