...
(tmp + rename, 0600). A full sweep also removes year shards that no
longer match any message; a ``years``-restricted sweep touches only the
requested years' shards.

Utility module: no CLI, no argparse, no main guard. The CLI layer
(`wicket.catalog.__main__`) dispatches into `sweep()`.
"""
...
def sweep(
    *,
    ...
    written/counted), ``shards`` (year shards), ``removed`` (stale shards
    deleted; full sweep only), ``parse_skips`` (FETCH responses that could
    ...
    UTC year, and afterwards removes shards outside that range.
    """
    if years:
