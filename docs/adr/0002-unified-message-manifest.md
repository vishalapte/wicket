...
> **Implementation status (2026-06-21):** the key, the year-sharded store, both
> mutating verbs (`catalog`, `fetch`), the reconcile, and the two-boolean state
> are implemented. `report` (read-only over the manifest) is a third verb added
> after this ADR and outside its scope. `--force` and `--attachments` are decided
> here but **not yet built** (no CLI flag on any verb); the `--force` relocation
> worker was removed pending reimplementation. Read the `--force` /
> `--attachments` sections below as design intent, not current behavior.
